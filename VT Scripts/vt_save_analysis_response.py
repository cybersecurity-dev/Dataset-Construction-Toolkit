...
def main(virustotal_api_key, binaries_directory, results_directory):    
    ...
    if not os.path.exists(binaries_directory):
        ...
        for filename in os.listdir(binaries_directory):
            ...
            # Check if the result already exists in the results directory
            result_file_path = os.path.join(results_directory, f"{sha256}.json")
            if os.path.exists(result_file_path):
                print(f"Result for {filename} already exists. Skipping VirusTotal request.")
                continue
        
            # Get result from VirusTotal using the calculated SHA256 hash
            result = get_virustotal_result(virustotal_api_key, sha256)
            
            # Save result to JSON file with filename as SHA256 hash in the specified directory
            save_to_json(result, sha256, results_directory)

        print(f"Results have been saved to {results_directory}.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4 :
        ...
        main(virustotal_api_key, dir_binary, file_output)
    else:
        print("python3" + __file__ + " <input directory> <output directory> <VT API KEY>")
